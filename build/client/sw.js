...
const aggregate = (items, extras) => {
  ...
  for (let item of items) {
    ...
    templates.push(html`<div class="row"><img src="${assets[
          `/images/icons/${item.type}/res/mipmap-xxhdpi/${item.type}.png`
        ]}" alt="${item.type}"><span>${item.startTime.toLocaleTimeString(navigator.language, {
          hour: "numeric",
          minute: "numeric",
        })} ${item.isDuration
          ...
          : ``} </span><a href="/${item.type}s/${item.id}/edit" class="edit row" title="Edit ${item.type} (${item.startTime})"><img src="${assets["/images/icons/ui/edit_24px.svg"]}"></a><button class="delete row" form="deleteForm${item.id}"><img src="${assets["/images/icons/ui/delete_24px.svg"]}"></button><form id="deleteForm${item.id}" class="deleteForm" method="POST" action="/${item.type}s/${item.id}/delete"><input type="hidden" name="return-url" value="${extras.referrer}"></form></div>`);
  }
    ...
    return html`${head(
      ...
      body(
        ...
        html`${extras.notFound
            ? html`<input type="hidden" name="data-loaded" value="${!!extras.notFound === false}">`
      ...
      body(
        ...
        html`${extras.notFound
            ...
            : ""}<div class="form"><form method="POST" id="deleteForm" action="/${data.type}s/${data.id}/delete"><input type="hidden" name="return-url" value="${extras.referrer}"></form><form method="POST" id="editForm" action="/${data.type}s/${data.id}/edit"><input type="hidden" name="return-url" value="${extras.referrer}"></form><div><div><label for="startDate">Start time: <input type="date" name="startDate" form="editForm" value="${getDate(
                    ...
                    )}"></label></div><div class="notes"><label for="notes">Notes: <textarea form="editForm" name="notes">
${sanitize(data.notes)}</textarea></label></div><div><div class="controls"><button form="deleteForm" class="delete"><img src="${assets["/images/icons/ui/delete_24px.svg"]}"></button> <input type="submit" form="editForm" value="Save"></div></div></div></div>`
      )
    )}`;
  }
}
...
class BaseView {
  ...
  async create(data, extras) {
    return html`${head(
      ...
      body(
        data,
        html`<div class="form"><form method="POST" action="/${data.type}s"><input type="hidden" name="return-url" value="${extras.referrer}"><div><label for="startDate">Start time: <input type="date" name="startDate" pattern="[0-9]{4}-[0-9]{2}-[0-9]{2}" placeholder="YYYY-MM-DD" value="${getDate(correctISOTime(new Date()))}"> <input type="time" name="startTime" pattern="[0-9]{2}:[0-9]{2}" placeholder="HH:MM" value="${getTime(correctISOTime(new Date()))}"></label></div>${!!extras.fieldsTemplates ? extras.fieldsTemplates : undefined}<div class="notes"><label for="notes">Notes:</label> <textarea name="notes"></textarea></div><div class="controls"><input type="submit" value="Save"></div></form></div>`
      )
    )}`;
  }
      ...
      body(
        ...
        html`${extras.notFound
            ...
            : ""}<div class="form"><form method="POST" id="deleteForm" action="/${data.type}s/${data.id}/delete"><input type="hidden" name="return-url" value="${extras.referrer}"></form><form method="POST" id="editForm" action="/${data.type}s/${data.id}/edit"><input type="hidden" name="return-url" value="${extras.referrer}"></form><div><div><label for="startDate">Start time: <input type="date" name="startDate" form="editForm" value="${getDate(
                    ...
                    )}"></label></div>${!!extras.fieldsTemplates ? extras.fieldsTemplates : undefined}<div class="notes"><label for="notes">Notes:</label> <textarea name="notes"></textarea></div><div class="controls"><button form="deleteForm" class="delete"><img src="${assets["/images/icons/ui/delete_24px.svg"]}"></button> <input type="submit" form="editForm" value="Save"></div></div></div>`
      )
    )}`;
  }
}
