...
let process = async (directory, origin, root, ignore) => {
  ...
  let files = allFiles.filter(file => file.isFile() && ignore.indexOf(path.join(directory.replace(root, '/'), file.name)) < 0 && file.name.startsWith('.') === false);
...
const go = async (directories, origin, ignore) => {
  let urls = new Set();
  for (let directory of directories) {
    const newUrls = await process(directory, origin, directory, ignore);
    urls = new Set([...urls, ...newUrls]);
  }
