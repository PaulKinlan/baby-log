...
let process = async (directory, origin, root, ignore) => {
  ...
  let files = allFiles.filter(file => file.isFile() && findAny(ignore, path.join(directory.replace(root, '/'), file.name)) == false && file.name.startsWith('.') === false);
...
const go = async (directories, origin, ignore) => {
  let urls = new Set();
  for (let directory of directories) {
    const newUrls = await process(directory, origin, directory, ignore.map(i => new RegExp(i)));
    urls = new Set([...urls, ...newUrls]);
  }
